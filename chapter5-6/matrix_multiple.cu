...
__global__ void matrix_multiple_kernel(float *A, float *B, float *C, size_t I, size_t K, size_t J, size_t sh_size) {
    ...
    size_t start_col = blockDim.x * blockIdx.x * COARSE_FACTOR + threadIdx.x;
    
    float cs[COARSE_FACTOR] = {0};
    for (int i = 0; i < (K + TILE_SIZE - 1) / TILE_SIZE; ++i) {
        ...
        for (int i = 0; i < COARSE_FACTOR; ++i) {
            size_t col = start_col + i * TILE_SIZE;
            if (cur_row < K && col < J) {
            ...
            for (int j = 0; j < TILE_SIZE; ++j) {
                cs[i] += As[threadIdx.y * TILE_SIZE + j] * Bs[j*TILE_SIZE + threadIdx.x];
            }
